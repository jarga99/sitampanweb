        ...
        <div class="col-lg-12">
            <div class="box">
                <div class="box-header with-border">
                    ...
                    <form id="form_pdf" action="{{ route('admin.puso.pdf_puso_perkebunan') }}" method="get"
                        style="display: none;">
                        @csrf
                    ...
                    <form id="form_excel" action="{{ route('puso.excel_perkebunan') }}" method="get"
                        style="display: none;">
                        @csrf
                        <input type="hidden" name="form_awal" id="form_awal">
                        <input type="hidden" name="form_akhir" id="form_akhir">
            ...
            @includeIf('admin.puso.form_add_perkebunan')
            ...
            <script>
                $(function() {
                    table = $('.table').DataTable({
                        ...
                        autoWidth: false,
                        ajax: {
                            url: '{{ route('admin.admin_puso_perkebunan.data') }}',
                        },
                        columns: [{
                                data: 'DT_RowIndex',
                                searchable: false,
                    ...
                    $(document).off("click", "#btn-search")
                        .on("click", "#btn-search", function(e) {
                            // e.preventhefault();
                            const months = ["January", "February", "Maret", "April", "Mei", "Juni", "Juli",
                                "Agustus",
                                "September", "Oktober", "November", "Desember"
                            ];
                            var tanggal_awal = new Date($('#tanggal_awal').val()).getDate() + ' ' + months[new Date(
                                    $(
                                        '#tanggal_awal').val()).getMonth()] + ' ' + new Date($('#tanggal_awal')
                                    .val())
                                .getFullYear();
                            var tanggal_akhir = new Date($('#tanggal_akhir').val()).getDate() + ' ' + months[
                                    new Date($(
                                        '#tanggal_akhir').val()).getMonth()] + ' ' + new Date($('#tanggal_akhir')
                                    .val())
                                .getFullYear();
                            ...
                            var p_tanggal_awal = new Date($('#tanggal_awal').val()).getFullYear() + '-' + $_s_bln +
                                '-' + $_s_tgl;
                            var p_tanggal_akhir = new Date($('#tanggal_akhir').val()).getFullYear() + '-' +
                                $_e_bln + '-' + $_e_tgl;
                            var content_title = `Daftar Data Puso Perkebunan ` + tanggal_awal + ` - ` +
                                tanggal_akhir;
                            $("th#_tm").html(null);
                            $("th#_tbm").html(null);
                            $("th#_ttm").html(null);
                            ...
                            $("th#prov").html(null);
                            $("th#harga").html(null);
                            table.ajax.url("{{ route('admin.admin_puso_perkebunan.data') }}?tanggal_awal=" +
                                p_tanggal_awal + "&tanggal_akhir=" + p_tanggal_akhir).load();
                ...
                function addForm() {
                    var url = "{{ route('admin.puso.create_perkebunan') }}";
