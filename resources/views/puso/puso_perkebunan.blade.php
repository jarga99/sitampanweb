        ...
        <div class="col-lg-12">
            <div class="box">
                <div class="box-header with-border">
                    <button onclick="updatePeriode()" class="btn btn-xs btn-info"><i class="fa fa-plus-circle"></i> Filter Periode</button>
                    <br>
                    <br>
                    ...
                    <form id="form_pdf" action="{{ route('puso.pdf_puso_perkebunan') }}" method="get" style="display: none;">
                        @csrf
            ...
            @includeIf('puso.form_add_perkebunan')
            @includeIf('puso.form')
        @endsection
            ...
            <script>
                $(function() {
                    table = $('.table').DataTable({
                        ...
                        autoWidth: false,
                        ajax: {
                            url: '{{ route('puso_perkebunan.data') }}',
                        },
                        columns: [
                            {
                                data: 'DT_RowIndex',
                    ...
                    .on("click","#btn-search",function (e) {
                        ...
                        var content_title = `Daftar Data Puso Perkebunan ` + tanggal_awal + ` - ` + tanggal_akhir;
                        $("th#_tm").html(null);
                        $("th#_tbm").html(null);
                        $("th#_ttm").html(null);
                        $("th#luas").html(null);
                        ...
                        $("th#prov").html(null);
                        $("th#harga").html(null);
                        table.ajax.url( "{{ route('puso_perkebunan.data') }}?tanggal_awal="+p_tanggal_awal+"&tanggal_akhir="+p_tanggal_akhir ).load();
                ...
                function addForm() {
                    var url = "{{ route('puso.create_perkebunan') }}";
                    $('#modal-form').modal('show');
                    $('#modal-form .modal-title').text('Tambah Data Puso Perkebunan');
                ...
                function editForm(id_produktivitas_puso) {
                    var url = "{{ url('puso/puso_perkebunan/update/') }}"+ "/" +id_produktivitas_puso;
                    $('#modal-form').modal('show');
                    $('#modal-form .modal-title').text('Edit Data Puso Perkebunan');
                    ...
                    $.ajax({
                        method: "get",
                        url: "{{ route('puso.edit_perkebunan') }}",
                        data: {
                            id_produktivitas_puso: id_produktivitas_puso
                        },
                ...
                function deleteSelected(url) {
                    if ($('input:checked').length > 1) {
                        if (confirm('Yakin ingin menghapus data terpilih?')) {
                            $.post(url, $('.form-puso-pajale').serialize())
                                .done((response) => {
                                    table.ajax.reload();
                                })
                                .fail((errors) => {
